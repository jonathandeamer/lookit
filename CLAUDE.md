...
- The local `~/bubbletea`, `~/bubbles`, `~/lipgloss`, `~/glow`, `~/soft-serve`, `~/crush` clones are reference material only; the versions resolved in `go.mod` are authoritative. After adding a Charm dependency, run `go mod tidy`.
