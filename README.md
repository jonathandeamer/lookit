...
With Go 1.26 or newer (the version in `go.mod`) you can install from source instead:
...
lookit opens on a startpage: a built-in catalog of finger communities and services, with your own bookmarks pinned above it. Press `↓` to browse it, `↵` to go, and `b` to bookmark whatever you're looking at — on a user list that bookmarks the host, so drill into someone before pressing `b` to bookmark the person. Bookmarks live in `~/.config/lookit/bookmarks` (or `$XDG_CONFIG_HOME/lookit/bookmarks`), one `<target>` per line, optionally followed by the date you last visited it (`2026-08-14`), so you can edit them by hand; anything after a `#` is a comment lookit keeps but never shows, and `catalog off` hides the built-in list entirely. Your shelf is ordered by that date — longest ago at the top, so what you have been neglecting is what you see first, with anything lookit has not seen you visit at the bottom; add `sort manual` to the file to keep your own order instead. lookit writes that date itself: visiting a target you have already bookmarked rewrites its line with today's date, keeping your comments, ordering, and indentation but normalising the spacing around the date; a second visit the same day leaves the file alone. You can edit the file while lookit is running — it re-reads it every time you return to the startpage, so a change shows up as soon as you press `h`. On first use, if that file does not exist, lookit creates it with a short comment header explaining all of the above and `jonathan@tilde.team` as a starter bookmark. Remove that line (or empty the file) to remove it permanently; deleting the file itself makes lookit initialize it again next time. Press `h` to return to the startpage from anywhere.
