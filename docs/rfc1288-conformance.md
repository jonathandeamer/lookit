...
| § | Requirement (verbatim, abridged) | Applies to | Status |
|---|---|---|---|
| 3.3 | "By default, this program **SHOULD filter** any unprintable data, leaving only printable 7-bit characters (ASCII 32–126), tabs (ASCII 9), and CRLFs." | **client** | ✅ Met by ingress-time control-character sanitization in finger.Query (`sanitize`, `finger/sanitize.go`). We intentionally depart from the literal "7-bit" wording to preserve UTF-8 — see "§3.3 resolution" below. |
...
| 3.1 | "An RUIP **SHOULD** protect itself against malformed inputs." | server, but spirit applies | ✅ The client applies the same defense: 1 MiB body cap, connect/read deadlines, context-cancel closing the connection, and reset-after-body handled (`client.go`), and control-char sanitization (`finger/sanitize.go`). |
...
**Result:** both client-side SHOULDs in §3.3 are decided — one implemented at
