...
| `Full reference: man lookit (package), man ./man/lookit.1 (archive or clone).` followed by `go install copies the binary only.` | `render/cli.go` (`Usage`) | Closing pointer in `-h`/`--help` output: packages install the page, archives and clones carry it in-tree, and `go install` does not include it. |
